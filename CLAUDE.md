...
- **Cookie system:** `fortiuser` (UUID), `fortiemail`, `forticustomer`, `fortievent`, `fortisites` (pipe-delimited lowercase site IDs). All 5-day rolling TTL. `forti_profile` in localStorage for resilience. **`fortiemail` has 2+ writers** (`analytics_checkin.html`'s form, `launchdemoform.html`'s "Change Email") — any validation/escaping fix to one must be applied to both, or the un-hardened writer reopens the same class of bug in the shared, cross-site storage. Never render it via `innerHTML`; `<`, `>`, `"` must always be excluded from what gets written, but not `'`/backtick (valid RFC 5322 local-part chars — fixed 2026-08-31, see memory).
...
- **`launchdemoform`'s backend has three independent locks, and a client-side fix can only ever cover one of them.** (1) The per-email rate limiter (`rate_limit_check`, burst-abuse only). (2) The dedup/claim entity (`try_claim`, keyed by email+lab+site) — permanently blocks any future attempt once `status` reaches `Completed`, **regardless of whether the credential has since expired**, unless the backend explicitly re-checks that (fixed 2026-09-01, see `fortinet-on-demand-labs-provisioning-and-tracking`'s `is_completed_claim_expired`). (3) The per-email history entity (populated by `record_completion`, added well after (1) and (2) already existed in prod) — an already-completed claim from before that feature shipped has no history entry, so any check keyed on history alone silently never applies to it. A real incident hit all three in sequence: the client's own `credentialsExpired()` guess was fixed first, then the rate limiter needed its own expiry-aware exemption, then the dedup lock (the actual remaining blocker) needed the same. **When "this should be unblocked now" still isn't, check which of the three locks you're actually looking at** — don't assume fixing one fixes the others.
- **When theorizing about which lock/entity is stuck, verify against the live backend Table Storage rather than reasoning from code alone.** `az storage entity query --table-name ProvisioningTaskHubInstances --account-name stcsewkspprov --account-key <key> --filter "PartitionKey eq '@provisionentity@<key>'"` (compute `<key>` from `compute_dedup_key`/`history_entity_key` in that repo's `src/idempotency.py`/`src/reuse.py`). Needs `--account-key` (`az storage account keys list`) — RBAC control-plane access (listing keys) does not imply Table data-plane access, and `az storage entity query --auth-mode login` fails with a permissions error even when `az account show` looks fully authenticated.
...
- **ContainerFlow/fortihugorunner shortcodes** currently include full `<!DOCTYPE html>` wrappers — this is invalid HTML when embedded in a Hugo page and causes rendering bugs. (`FTNThugoFlow` had the same problem and was retired org-wide 2026-08-25 — see below.)
...
- **The dev image build is tied to the `dev` branch by name, asserted in two places that must stay in agreement:** `.github/workflows/image-build-push-dev.yaml:6` (push trigger) and `Dockerfile` (`ADD …CentralRepo.git#dev`). Change one without the other and the dev image builds a branch nothing pushes to. **The Dockerfile's `prod` stage independently pins `#main`, not the dev branch** — this is why a future rename of `dev` itself can never break any downstream repo's `--target=prod` build (only a manual `--target dev` local build is exposed; confirmed across all 52 downstream repos during the 2026-08-25 `prreviewJune23`→`dev` rename). `prreviewJuly23` is a decoy — commits behind `main`, last commit 2026-06-11, referenced by no workflow and no Dockerfile stage. Never branch from it.
- **A push-triggered workflow's `on.push.branches` filter is evaluated using the workflow YAML content in the commit being pushed, matched against the actual ref name** — not the filter that was live before the push. Editing a trigger's branch name in the same commit you push to the *old*-named branch means that commit's own new filter no longer matches, so nothing fires — not a failure, a structural fact discovered while renaming `prreviewJune23`→`dev`. There is no way to verify an old-name trigger still works once you've edited it; the only real verification is a push *after* the rename completes.
- **`batch_repo_update.py` deleting a file from `FILES_TO_DELETE` is not the same as retiring it.** It has included `layouts/shortcodes/FTNThugoFlow.html` for years, correctly stripping the implementation from every repo it touches — but nothing ever updated the *content* that still called it (inherited from `UserRepo`'s own default `content/_index.md`), so repos silently accumulated a hard Hugo build failure with no warning. Before adding any file to `FILES_TO_DELETE`, grep the org for content that still references it (shortcode calls, not just file copies) and fix that too, or the same bug recurs with a different shortcode. `FTNThugoFlow` retired org-wide 2026-08-25 (`docs/plans/2026-08-25_Jeff-Kopko_centralrepo-branch-rename.md`).
- **`UserRepo` is a real GitHub template repo (`is_template: true`) with a protected `main`** (PR required, same posture as CentralRepo's own) — any automation touching it needs a PR flow, not a direct push. It is also the single propagation point for both fixes *and* breakage into every future new workshop repo: its own default content and shortcodes get cloned verbatim into everything built from it.
- **A green "Deploy static content to Pages" checkmark from before 2026-08-25 is not proof a site actually built.** Every downstream repo's `static.yml` used `docker wait "$CONT_ID"` with no exit-code capture until that date, so a failed Hugo build inside the container was reported as CI success. Fixed org-wide; doing so surfaced 8 previously-invisible broken deploys across 7 repos + `UserRepo` (a shortcode removed from CentralRepo in 2023 with content never updated, a malformed-quotes markdown link, a duplicate YAML key, a misplaced shortcode file, a deprecated `quizdown` reference, and one repo with GitHub Pages never enabled at all). Don't trust an old green run as evidence a site is currently healthy.
...
- **An `XpertsNNBanner` shortcode is a draw.io "Embed HTML" export, not hand-written markup — three nested encoding layers, and hand-editing any of them breaks the whole div.** The `data-mxgraph` attribute holds HTML-entity-encoded JSON (`{"xml": "...", ...}`); the JSON's `"xml"` value is that same entity-encoding applied *again* to the raw mxGraphModel XML (so the XML's own `<`/`>`/`"` show up doubled, e.g. `&amp;lt;`); and each `mxCell`'s `value="..."` attribute (the actual line1/2/3 text, styled via an inner `<font>` tag) is separately XML-attribute-escaped before that whole XML string gets its outer encoding pass. Reproduce this programmatically (build the raw XML, `json.dumps` it, then `html.escape`) rather than editing the giant single-line div by hand — verified byte-for-byte against `Xperts25Banner.html` when building `Xperts26Banner.html` this way. The background image itself is embedded directly as `image=data:image/png,<base64>` inside the image cell's `style`, not referenced as a static file — a source PNG kept under `static/images/` (naming convention: `xperts-NNNN-background.png`) is for future edits only, nothing in the shortcode reads it.
- **Promoting `dev` → `main` here routinely trips the CI-skip-token pre-flight on `gh-merge-verify`, even when `git diff origin/main origin/dev` shows a small, clean diff.** GitHub's PR compare walks *commit ancestry*, not content — old `[skip ci]`-tagged plan/log commits (e.g. from the 2026-08-25 branch-rename work) can still show up in the base...head commit list your PR would squash together, well after their file changes are already reflected in `main` some other way. `gh-merge-verify`'s pre-flight catches this (it recomputes the true compare diff, so a false positive is unlikely) — the fix is `-- --subject "..." --body "..."`, which replaces the default squash message (title + body + every commit message concatenated) entirely, rather than `--allow-skip-token`.
