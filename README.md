...
- **`switchTab()` persists only on a real click.** It reads the implicit global `event` (`theme.js:120`), so anything that calls it programmatically switches the tab visually but never writes `localStorage` — the choice silently fails to survive navigation. Do not work around this by writing the tab-selection key yourself; that forks the theme's contract. Observe the `.active` class with a `MutationObserver` instead, which also catches the theme's own `restoreTabSelections()` on load. `layouts/shortcodes/pathtabs.html` does it this way.
