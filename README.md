...
- `disableInlineCopyToClipBoard`: Optional boolean, default `false`. `true` removes the copy icon from all inline code. By default relearn adds one only to inline code longer than 5 characters (so `` `system` `` gets one and `` `user` `` does not), which reads as random in prose and tables. Code-block copy buttons are unaffected. Use this in workshops whose runnable commands all live in code blocks.
